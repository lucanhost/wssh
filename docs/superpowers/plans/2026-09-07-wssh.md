...
```
Note: `os/user.User` has NO `Shell` field — the shell travels as a separate string param (from the auth callback's `shell` extension, populated by `lookupShell` in auth.go from Task 5). `(add `net` import; `s.wg.Add(1)` before `go` — never Add inside the goroutine).
